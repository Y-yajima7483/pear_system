...
- **Never implement API and frontend simultaneously**: Do not implement both backend (Laravel API) and frontend (Next.js) in a single task. Always work in separate phases: API implementation first, then frontend implementation
- **Avoid `any` type in frontend code**: Use proper TypeScript types whenever possible. Only use `any` as a last resort when typing is genuinely impractical.
